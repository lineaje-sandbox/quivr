...
class QuivrQARAGLangGraph:
    ...
    def invoke_structured_output(
        ...
        try:
            ...
            prompt = _gr_client.enforce(_gr_site, prompt, content_type='application/json', variable_name='prompt', source_file=__file__, before_line=1194)
            return structured_llm.invoke(prompt)
        except openai.BadRequestError:
            structured_llm = self.llm_endpoint._llm.with_structured_output(output_class)
            # LINEAJE: enforce() `prompt` at agent->llm pre_model — scan flagged AI_DAT_SEC_029 (Enforce decision logging, audit trail, and forensic readiness for AI-driven actions.). Mask/block; do not remove without review. site_id='site:sha256:f4f0fd943ff72da53ae37cb37e098b5ac7e668fd61298066edfb51599b5962f6'
            _gr_client = _lineaje_load_gr_client()
            _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:f4f0fd943ff72da53ae37cb37e098b5ac7e668fd61298066edfb51599b5962f6', phase='pre_model', boundary={'source': 'agent_message', 'sink': 'model'}, candidate_policies=[], fail_mode='ALLOW_WITH_AUDIT', source_type='agent', destination_type='llm')
